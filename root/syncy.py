...
__VERSION__ = '2.5.3'
__DEBUG__ = False
__author__ = "WishInLife <wishinlife@qq.com> AND ZiShuo <www@zishuo.uu.me>"
...
class SyncY:
        ...
        if 'refresh_token' not in SyncY.syncytoken or SyncY.syncytoken['refresh_token'] == '' or (len(self.__argv) != 0 and self.__argv[0] in ['sybind', 'cpbind']):
            ...
            if (('access_token' not in SyncY.syncytoken or SyncY.syncytoken['access_token'] == '') and len(self.__argv) == 0) or (len(self.__argv) != 0 and self.__argv[0] == 'sybind'):
                ...
                if retcode != 200 or 'error_code' in responses:
                    ...
                    print(' ')
                    sys.exit(0)
                SyncY.syncytoken['device_apps'] = responses['device_apps']
    ...
    def __start_sync(self):
        ...
        for i in range(len(SyncY.syncpath)):
            ...
            remotepath = self.__catpath(SyncY.syncpath[str(i)]['remotepath'])# SyncY.pcsroot,SyncY.pcsroot,SyncY.syncytoken['device_apps'],
            if remotepath.find(SyncY.syncytoken['device_apps']) <= 0:
                apps = remotepath.split('/');
                remotepath = remotepath.replace(apps[2],SyncY.syncytoken['device_apps'])
    ...
    def start(self):
        ...
        if len(self.__argv) == 0:
            ...
            if len(self.__argv) == 2:
                pcsdir = self.__catpath(self.__argv[1])#SyncY.pcsroot,SyncY.syncytoken['device_apps'], 
            else:
                pcsdir = ''#SyncY.pcsroot,SyncY.syncytoken['device_apps']
    ...
    def __download_file(self):
        ...
        if self.__rsize > 0:
            if self.__rsize <= self.__blocksize + 1048576:
                ...
                sycurl = SYCurl()
                retcode, responses = sycurl.request('https://www.baidupcs.com/rest/2.0/pcs/stream', {'method': 'download', 'access_token': SyncY.syncytoken['access_token'], 'path': self.__pcspath}, '0-%d' % (self.__rsize - 1), 'GET', SYCurl.Download, '%s.syy' % self.__filepath)
        ...
        try:
            ...
            while True:
                ...
                if SyncY.synctask[self.__fnmd5][0][0] == 'upload':
                    ...
                    retcode, responses = sycurl.request('https://www.baidupcs.com/rest/2.0/pcs/stream', {'method': 'download', 'access_token': SyncY.syncytoken['access_token'], 'path': self.__pcspath}, '%d-%d' % (startpos, endpos), 'GET', SYCurl.Download, '%s.syy' % self.__filepath)
                    if (retcode != 200 and retcode != 206) or responses != '':
                        if __DEBUG__:
